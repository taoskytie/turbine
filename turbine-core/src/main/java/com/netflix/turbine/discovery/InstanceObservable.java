...
//            .newScheduledThreadPool(2000);
    ...
    public void start(InstanceDiscovery iDiscovery) {
        ...
        System.out.println("myinstanceobservable.start.iDiscover"+instanceDiscovery.toString());
...
//                List<Instance> mynewList = newList;
                ...
                System.out.println("instanceObservation.previousList"+previousList.toString());
                System.out.println("instanceObservation.newList"+newList.toString());
//                List<Instance> mypreviousList = new ArrayList<Instance>(currentState.get().hostsUp);
                if(!(previousList.containsAll(newList)&&newList.containsAll(previousList)) && currentState.get().hostsUp.size() > 0){
                    MyRestart myrestart=new MyRestart();
                    myrestart.start();
//                    return;
                }
                ...
                System.out.println("wowatcher1*"+observers.values()+"*");
                for(InstanceObserver watcher: observers.values()) {
                    if(currentState.get().hostsUp.size() > 0) {
                        try {
                            Date rightnow=new Date();
                            System.out.println("wowatcher["+rightnow+"]"+currentState.get().hostsUp);
                            //add by fxc
     ...
     */
    private  TimerTask producer = new TimerTask() {
        ...
        public void run() {
            ...
            try {
                ...
                List<Instance> previousList = new ArrayList<Instance>(currentState.get().hostsUp);
                System.out.println("instanceObservation.previousList"+previousList.toString());
                System.out.println("instanceObservation.previousList"+previousList.size());
                ...
                // Notify watchers of fleet changes
                if(previousList.size()>0 && currentState.get().hostsUp.size() > 0){
                    MyRestart myrestart=new MyRestart();
                    myrestart.start();
                }
                System.out.println("wowatcher1*"+observers.values()+"*");
    ...
    private List<Instance> getInstanceList() throws Exception {
        ...
        if(instanceDiscovery != null) {
            System.out.println("instanceObservable.getInstanceList.instance.addall");
